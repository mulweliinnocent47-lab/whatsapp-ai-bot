...
// use dynamic node-fetch import to be compatible with newer node versions
const fetch = (...args) =>
...
app.post("/api/webhook", async (req, res) => {
  try {
    // quick 200 to acknowledge receipt (Meta expects 200)
    res.sendStatus(200);
    ...
    const messageText =
      msg?.text?.body ||
      msg?.button?.text || // fallback for button replies
      msg?.interactive?.type === "button_response"
    ...
    // --- Call Supabase AI function with the fetch you provided ---
    // Headers include the Supabase Authorization Bearer token
    ...
    // Try parsing JSON first, otherwise get text
    let aiReply;
    try {
      const json = await aiResponse.json();
      // If your function returns { reply: "..." } or similar, pick the right field:
      if (typeof json === "string") {
        ...
        // fallback: stringify the whole response (shorten if long)
        aiReply = JSON.stringify(json).slice(0, 3000);
      }
    } catch (e) {
      // not JSON — read as plain text
      aiReply = await aiResponse.text();
    }
